...
const Dashboard = () => {
  ...
  const parseQuantidade = (q) => {
    const n = parseInt(q, 10);
    return isNaN(n) ? 0 : n;
  };

  const totalEstoque = produtos.reduce((acc, item) => acc + parseQuantidade(item.quantity), 0);
  const produtosComEstoqueBaixo = produtos.filter(item => parseQuantidade(item.quantity) < 10).length;
  const produtosZerados = produtos.filter(item => parseQuantidade(item.quantity) === 0);
  const topCriticos = [...produtos]
    .sort((a, b) => parseQuantidade(a.quantity) - parseQuantidade(b.quantity))
    .slice(0, 5);
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <ul className="text-sm text-gray-700 list-disc list-inside">
              ...
              {produtosZerados.map(p => (
                <li key={p.id}>{p.name || "Sem nome"}</li>
              ))}
            </ul>
          </div>
            ...
            <ul className="divide-y divide-gray-100">
              {topCriticos.map(p => (
                <li key={p.id} className="py-2 flex justify-between text-sm text-gray-800">
                  <span>{p.name}</span>
                  <span className="font-medium text-red-600">{parseQuantidade(p.quantity)} un.</span>
                </li>
              ))}
            </ul>
          </div>
